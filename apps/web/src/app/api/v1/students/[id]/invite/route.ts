...
import { issueStudentInvite, StudentInviteStateError } from "@/modules/students/invite-service";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (!isGuardianRole(session.role)) {
      logAccessDenied("student_invite_requires_guardian_role", {
    ...
    const result = await issueStudentInvite({
      studentId: id,
    ...
    if (error instanceof StudentInviteStateError) {
      return apiError(409, "CONFLICT", "이미 활성화된 학생 계정입니다. 재설정 후 새 초대코드를 발급하세요.");
    }
