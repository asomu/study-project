...
import { parseGuardianWrongNoteDashboardQuery } from "@/modules/wrong-note/schemas";
...
export async function GET(request: Request) {
  try {
    ...
    if (!isGuardianRole(session.role)) {
      logAccessDenied("guardian_wrong_note_dashboard_requires_guardian_role", {
    ...
    const parsed = parseGuardianWrongNoteDashboardQuery(requestUrl);
